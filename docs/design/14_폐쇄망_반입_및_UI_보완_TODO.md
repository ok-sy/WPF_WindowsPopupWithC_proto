...
- [ ] [WPF 단일 EXE 자동 업데이트](17_WPF_단일_EXE_자동업데이트_TODO.md) — self-contained EXE + PowerShell 교체, 버전 3단계 기본 정책, 서버 그룹별 시간 분배, 다운로드 검증·재실행·복구. 현재는 문서만 작성했다. 사용자가 이 과제를 직접 시작하자고 명시하기 전에는 이행하지 않는다. 다른 TODO 완료나 일반적인 개선·검증·배포 요청은 착수 근거가 아니다. 기능의 긴급 배포 정책과 이 TODO의 개발 우선순위는 별개다.
