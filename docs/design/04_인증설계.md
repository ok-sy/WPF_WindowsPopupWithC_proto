...
| 추가 (도구·테스트) | `popup-frameWork/MockSso` — 임시 SSO 프로그램(C# `HttpListener`, 기본 Negotiate 도전, `MAIN_USER_ID`/`MAIN_USER_CLASSI_CODE` XML). WPF 관리 화면 **"SSO 로그인 테스트"** 버튼(`SsoAuthHeaderProvider.TestLoginAsync`) + 실행 인자 `--show-main` |
