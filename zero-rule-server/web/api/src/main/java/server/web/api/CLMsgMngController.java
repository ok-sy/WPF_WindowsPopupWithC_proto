...
public class CLMsgMngController extends ApiBaseController {
    ...
    @Autowired
    private CLMsgMngService clMsgMngService;

    @Autowired
    private ICLMsgRepository msgRepository;
    ...
    public CLNewApiResponse<CLMsgMngPayloads.FindPageResponse> list(
        ...
        // 조회된 데이터가 없는 경우
        if (tempData == null|| tempData.getElements().isEmpty()) {
    ...
    public synchronized CLNewApiResponse<CLMsgMngPayloads.InsertResponse> create(
        ...
        // map에서 service 호출시 local에선 정상 동작하나, 서버에서 오류발생~ jdk11 문제로 여겨짐...
        // 각 메시지에 대해 insert를 호출하고 결과를 세어봅니다.
//        insertCnt = (int) msgArr.stream()
//            .map(el -> {
...
//            .count();
        ...
        for (int i = 0; i < msgArr.size(); i++) {
            ...
            ParamsCLMsgMng.InsertArr el = msgArr.get(i);
            ParamsCLMsgMng.Insert insert = ParamsCLMsgMng.Insert.builder()
                .msgClsf(el.getMsgClsf())
                .tskClsfCd(el.getTskClsfCd())
                .teamId(el.getTeamId())
